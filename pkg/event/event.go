...
func New(reason string, payload interface{}) *Event, error {
	...
	return &Event{
		...
		serializerType: valueobjects.SerializerTypeJson
	}
}

func NewWithSerializerType(reason string, payload interface{}, serializerType valueobjects.SerializerType) *Event, error {
	serializer = valueobjects.MatchedSerializers(serializerType)
	if err != nil {
		return nil, err
	}
	return &Event{
...
func(e *Event) GetIndentifer() string {
	return fmt.Sprintf("%s-%s", e.aggregateId, e.aggregateType)
}